...
option_list <- list(
  make_option(c("-d", "--workdir"), type = "character", default = ".",
              help = "Directory containing *_hmmIBD_ibd_winXXkb.tsv and *_hmmIBD_fraction.tsv",
              metavar = "character"),
  ...
  make_option(c("--window_size"), type = "numeric", default = 50000,
              help = "Sliding window size used in summarise step [default %default]",
              metavar = "numeric"),
  make_option(c("--quantile_cutoff"), type = "numeric", default = 0.95,
              help = "Quantile cutoff for high-IBD windows [default %default]",
              metavar = "numeric"),
  make_option(c("--remove_chr"), type = "character", default = "Pf3D7_API_v3,Pf3D7_MIT_v3",
  ...
  make_option(c("-o", "--outdir"), type = "character", default = NULL,
              help = "Output directory for plots (default: workdir/win_XXkb)",
              metavar = "character")
)
...
q_cut          <- opt$quantile_cutoff
rm_chr_str     <- opt$remove_chr
pattern        <- opt$regex_chr
groupid        <- opt$regex_groupid

if (is.null(ref_index) || !file.exists(ref_index)) {
  stop("ERROR: --ref_index is required and must exist.\n", call. = FALSE)
}
if (is.null(gene_file) || !file.exists(gene_file)) {
  stop("ERROR: --gene_product is required and must exist.\n", call. = FALSE)
}
if (is.null(suffix)) {
  stop("ERROR: --suffix must match the prefix used in summarise_hmmibd_windows.R.\n",
       call. = FALSE)
}
...
if (is.null(opt$outdir)) {
  outdir <- file.path(workdir, paste0("win_", window_size / 1000, "kb"))
} else {
  outdir <- opt$outdir
}
...
message("Suffix: ", suffix, "  window_size: ", window_size)

# ─────────────────────────────────────────────────────────────
# Load combined IBD summaries
# ─────────────────────────────────────────────────────────────
...
if (!file.exists(ibd_file)) {
  stop("IBD window file not found: ", ibd_file, "\n", call. = FALSE)
}
if (!file.exists(frac_file)) {
  stop("Fraction IBD file not found: ", frac_file, "\n", call. = FALSE)
}
...
# If there are no data, bail
if (nrow(combined_ibd) == 0L) {
  stop("combined_ibd is empty; nothing to plot.\n", call. = FALSE)
}
if (nrow(fraction_ibd) == 0L) {
  stop("fraction_ibd is empty; nothing to plot.\n", call. = FALSE)
}

# Deduce category order from data itself
category_order <- combined_ibd$category %>%
  unique() %>%
  sort()

# ─────────────────────────────────────────────────────────────
# Process FAI (chromosome layout)
# ─────────────────────────────────────────────────────────────
...
if (!is.null(rm_chr_str) && nzchar(rm_chr_str)) {
  ...
  present_rm <- intersect(rm_chr, unique(fai$chr))
  if (length(present_rm) > 0) {
    fai <- fai %>% filter(!chr %in% present_rm)
  } else {
    warning("None of the chromosomes in --remove_chr found in FAI; using all.\n")
  }
} else {
  message("No chromosomes removed from FAI (API/mito may be included).")
}

fai <- fai %>%
  mutate(chr_num = as.numeric(str_match(chr, pattern)[, groupid])) %>%
  arrange(chr_num) %>%
  mutate(tr_chr = lag(cumsum(end_chr), default = 0),
...
# ─────────────────────────────────────────────────────────────
# Boxplot of pairwise IBD fractions
# ─────────────────────────────────────────────────────────────
...
ggsave(file.path(outdir, "ibd_fraction_boxplot.png"), g_box, width = 8, height = 6, dpi = 300)

# ─────────────────────────────────────────────────────────────
# Genome-wide IBD line plot
# ─────────────────────────────────────────────────────────────
combined_ibd_tr <- combined_ibd %>%
  filter(!is.na(fraction), !is.na(category), !is.na(chr), !is.na(win_start)) %>%
  mutate(chr = as.numeric(chr)) %>%
  left_join(transpose_chr, by = "chr") %>%
  mutate(pos_bp_ed = as.numeric(win_start) + tr_chr,
         category = factor(category, levels = category_order))

signif_cutoff <- quantile(combined_ibd_tr$fraction, probs = q_cut, na.rm = TRUE)
...
ggsave(file.path(outdir, "ibd_genomewide_fraction_cleaned.png"),
       p_clean, width = 12, height = 15, dpi = 300)

# ─────────────────────────────────────────────────────────────
# Overlap high-IBD windows with genes
# ─────────────────────────────────────────────────────────────
