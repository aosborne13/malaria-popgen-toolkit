...
"""
Haplotype maps for drug-resistance genes by region or globally.
...
- Plots pie-chart haplotype maps with GeoPandas (geodatasets or Natural Earth fallback)

Supported map scopes:
- africa
- samerica / south_america
- seasia / southeast_asia
- oceania
- seasia_oceania / asia_pacific
- global / world / all
...
def _region_geodf(region: str, cache_dir: str | None = None) -> gpd.GeoDataFrame:
    ...
    if region in ("southeast_asia", "seasia"):
        ...
        return world[world[name_col].isin(seasia_countries)].copy()
    ...
    if region == "oceania":
        ...
        return world[world[name_col].isin(oceania_countries)].copy()

    if region in ("seasia_oceania", "asia_pacific"):
        combined = {
            "Myanmar",
            "Thailand",
            "Laos",
        ...
        return world[world[name_col].isin(combined)].copy()

    if region in ("global", "world", "all"):
        return world.copy()
...
def run(
    ...
    """
    Build and plot haplotype pies for a region or globally; export per-gene haplotype frequencies.
    """
    ...
    meta[country_col] = meta[country_col].astype(str).str.strip()
    meta = meta[meta[sample_col].ne("") & meta[sample_col].notna()]
    meta = meta[meta[country_col].ne("") & meta[country_col].notna()]
    ...
    for country, dfc in meta.groupby(country_col):
        ...
        if missing:
            print(
                f"[WARN] {country}: {len(missing)} metadata samples not in VCF. "
    ...
    all_hap_freqs: Dict[str, Dict[str, Counter]] = {}
    ...
    for gene, country_counts in all_hap_freqs.items():
        ...
        for user_country, counter in country_counts.items():
            ...
            if canon is None:
                ...
                continue
            cc2.setdefault(canon, Counter()).update(counter)
        remapped[gene] = cc2
...
# Plotting (2x2 panels)
# ---------------------------------------------------------------------
def _plot_region_pies(
    hap_dict: Dict[str, Dict[str, Counter]],
    ...
    # Tighter extents for SE Asia-focused maps
    if region_l in ("seasia", "southeast_asia"):
        xmin, xmax = 92, 155
        ymin, ymax = -12, 25
        padx, pady = 1.5, 1.5
    elif region_l in ("seasia_oceania", "asia_pacific"):
        xmin, xmax = 92, 180
        ymin, ymax = -20, 25
        padx, pady = 2.0, 2.0
    else:
    ...
    map_w = (xmax - xmin)
    if region_l in ("global", "world", "all"):
        radius_deg = 2.5
    elif region_l in ("seasia", "southeast_asia"):
        radius_deg = 1.8
    elif region_l in ("seasia_oceania", "asia_pacific"):
        radius_deg = 1.9
    else:
        radius_deg = max(0.6, 0.036 * map_w)
