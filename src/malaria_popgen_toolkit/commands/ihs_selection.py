...
def run(
    ...
    Supports:
      ...
      - optional subgroup (e.g. year)
      - per-group Manhattan plots
      - DR-gene focused annotation
    """
    ...
    cmd = [
        ...
        "--fws_th", str(fws_th),
        "--maf", str(maf),
        "--rehh_min_perc_hap", str(rehh_min_perc_hap),
        "--rehh_min_perc_mrk", str(rehh_min_perc_mrk),
        "--na_char", na_char,
        "--regex_chr", regex_chr,
        "--regex_groupid", str(regex_groupid),
        "--threads", str(threads),
        "--min-maf-ihs", str(min_maf_ihs),
        "--freqbin", str(freqbin),
        "--ihs-thresh", str(ihs_thresh),
        "--logp-thresh", str(logp_thresh),
    ]
    ...
    if subgroup_col:
        cmd += ["--subgroup_col", subgroup_col]

    if focus_pop:
        cmd += ["--focus-pop", focus_pop]
